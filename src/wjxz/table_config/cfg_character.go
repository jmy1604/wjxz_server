...
type CfgCharacterMgr struct {
	Map map[int32]*XmlCharacterItem
}

func (this *CfgCharacterMgr) GetCat(id int32) *XmlCharacterItem {
	item, o := this.Map[id]
	if !o {
		return nil
	}
...
func (this *CfgCharacterMgr) Init() bool {
	data, err := ioutil.ReadFile("../game_data/Character.xml")
	if nil != err {
		log.Error("ChgCharacterMgr Init read file err[%s] !", err.Error())
		return false
	}
	...
	if nil != err {
		log.Error("ChgCharacterMgr Init xml Unmarshal failed error [%s] !", err.Error())
		return false
	}
