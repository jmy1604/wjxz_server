...
type CfgBuildingMgr struct {
	Map   map[int32]*XmlBuildingItem
	Suits map[int32]*SuitItems
}

func (this *CfgBuildingMgr) Init() bool {
	data, err := ioutil.ReadFile("../game_data/Building.xml")
	if nil != err {
		log.Error("CfgBuildingMgr Init read file err[%s] !", err.Error())
		return false
	}
	...
	if nil != err {
		log.Error("CfgBuildingMgr Init xml Unmarshal failed error [%s] !", err.Error())
		return false
	}
	...
	for idx := int32(0); idx < tmp_len; idx++ {
		...
		if len(tmp_item.MapSizes) < 2 {
			log.Error("CfgBuildingMgr Init [%d] mapsize[%s] error", tmp_item.Id, tmp_item.MapSizeStr)
			return false
		}
