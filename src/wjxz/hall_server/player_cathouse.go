...
func (this *Player) building_setdone(building_id int32) int32 {
	is_done, o := this.db.Buildings.GetIsDone(building_id)
	if !o {
		log.Error("Player[%v] building[%v] not exist", this.Id, building_id)
		return int32(msg_client_message.E_ERR_BUILDING_NOT_EXIST)
	}

	if is_done > 0 {
		log.Warn("Player[%v] cathouse[%v] is no need to set done", this.Id, building_id)
	...
	if building.Type == PLAYER_BUILDING_TYPE_CAT_HOME {
		...
		if building.BuildTime > 0 {
			...
			if int32(time.Now().Unix())-create_unix < building.BuildTime {
				return int32(msg_client_message.E_ERR_BUILDING_IS_IN_CONSTRUCTION)
			}
		}
	}
	...
	response := &msg_client_message.S2CBuildingSetDoneResult{}
