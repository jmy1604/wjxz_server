...
func foster_cats_cmd(p *Player, args []string) int32 {
	if len(args) < 1 {
		log.Error("参数[%v]不够", len(args))
		return -1
	}
	...
	friend_id, err = strconv.Atoi(args[0])
	if err != nil {
		log.Error("转换参数好友ID[%v]失败[%v]", args[0])
		return -1
	}

	return p.foster_data_by_player_id(int32(friend_id))
}

func foster_set_cmd(p *Player, args []string) int32 {
	if len(args) < 2 {
		log.Error("参数[%v]不够", len(args))
		return -1
	}

	var friend_id, cat_id, card_id int
	var err error
	friend_id, err = strconv.Atoi(args[0])
	if err != nil {
		log.Error("转换参数好友ID[%v]失败[%v]", args[0], err.Error())
		return -1
	}
	cat_id, err = strconv.Atoi(args[1])
	if err != nil {
		log.Error("转换参数猫ID[%v]失败[%v]", args[1], err.Error())
		return -1
	}
	if len(args) > 2 {
		card_id, err = strconv.Atoi(args[2])
		if err != nil {
			log.Error("转换参数寄养卡ID[%v]失败[%v]", args[2], err.Error())
			return -1
		}
	}

	return p.foster_cat(int32(friend_id), int32(cat_id), int32(card_id))
}

func foster_out_cmd(p *Player, args []string) int32 {
	if len(args) < 1 {
		log.Error("参数[%v]不够", len(args))
		return -1
	}
	...
	for i := 0; i < len(args); i++ {
		cat_id, err = strconv.Atoi(args[0])
		if err != nil {
			log.Error("转换猫ID[%v]失败[%v]", args[i], err.Error())
			return -1
		}
		cat_ids = append(cat_ids, int32(cat_id))
	}

	return p.foster_cat_finish(cat_ids)
}

func foster_incomes_cmd(p *Player, args []string) int32 {
	incomes := p.foster_get_incomes()
	if incomes != nil && len(incomes) > 0 {
		log.Debug("!!!!!! Player[%v] get incomes: ", p.Id)
		for _, income := range incomes {
			log.Debug("      income: cat_id[%v] card_id[%v] friend_id[%v] friend_name[%v]", income.GetCatId(), income.GetCardId(), income.GetFriendId(), income.GetFriendName())
		}
	}
	return 1
}
...
var test_cmd2funcs = map[string]test_cmd_func{
	...
	"foster_cats":               foster_cats_cmd,
	"foster_set":                foster_set_cmd,
	"foster_out":                foster_out_cmd,
	"foster_incomes":            foster_incomes_cmd,
}
