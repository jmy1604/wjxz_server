...
type XmlFormulaItem struct {
	...
	UnlockAreaCond string `xml:"UnLockAreaCond,attr"`
	UnlockAreas    []int32
}
...
func (this *FormulaTableMgr) Load() bool {
	...
	for idx := int32(0); idx < tmp_len; idx++ {
		...
		// 需要解锁区域
		tmp_item.UnlockAreas = make([]int32, 0)
		d = parse_xml_str_arr(tmp_item.UnlockAreaCond, ",")
		if d != nil {
			for i := 0; i < len(d); i++ {
				tmp_item.UnlockAreas = append(tmp_item.UnlockAreas, d[i])
			}
		}
