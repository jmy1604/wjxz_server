...
func (this *CfgExpeditionMgr) Init() bool {
	if !this.LoadExpediton() {
		return false
	}
...
func (this *CfgExpeditionMgr) LoadExpediton() bool {
	data, err := ioutil.ReadFile("../game_data/SearchTask.xml")
	if nil != err {
		log.Error("CfgExpeditionMgr Init read file err[%s] !", err.Error())
		return false
	}
	...
	if nil != err {
		log.Error("CfgExpeditionMgr Init xml Unmarshal failed error [%s] !", err.Error())
		return false
	}
	...
	for idx := int32(0); idx < tmp_len; idx++ {
		...
		if len(tmp_item.FixRewards)%2 != 0 {
			log.Error("CfgExpeditionMgr init FixRewardsStr[%s] error !", tmp_item.FixRewardsStr)
			return false
		}
...
func (this *CfgExpeditionMgr) LoadEvent() bool {
	data, err := ioutil.ReadFile("../game_data/SearchEvent.xml")
	if nil != err {
		log.Error("CfgExpeditionMgr LoadEvent read file error(%s) !", err.Error())
		return false
	}
	...
	err = xml.Unmarshal(data, tmp_cfg)
	if nil != err {
		log.Error("CfgExpeditionMgr loadEvent unmarshal failed (%s) !", err.Error())
		return false
	}
	...
	for idx := int32(0); idx < tmp_len; idx++ {
		...
		if len(tmp_item.DropIds)%3 != 0 {
			log.Error("CfgExpeditionMgr LoadEvent dropid[%s] error !", tmp_item.DropIdStr)
			return false
		}
...
func (this *CfgExpeditionMgr) RandNWithExistIds(cur_ids map[int32]bool, p_lvl, need_n int32) (ret_tasks []*XmlExpeditionItem) {
	if nil == cur_ids || need_n < 1 {
		log.Error("CfgExpeditionMgr RandNWithExistIds param error %v!", need_n)
		return nil
	}

	log.Info("CfgExpeditionMgr RandNWithExistIds %v %d %d", cur_ids, p_lvl, need_n)
	...
	if total_weight <= 0 {
		log.Error("CfgExpeditionMgr RandNWithExistIds total_weight[%d] <= 0 !", total_weight)
		return nil
	}
...
func (this *CfgExpeditionMgr) RandEvent(eventid int32) *XmlExpeditionEventItem {
	event_cfg := this.Id2Event[eventid]
	if nil == event_cfg {
		log.Error("CfgExpeditionMgr RandEvent failed to find cfg [%d]!", eventid)
		log.Info("当前所有事件")
		for id, event := range this.Id2Event {
			log.Info("	时间[%d] [%v]", id, *event)
		}
