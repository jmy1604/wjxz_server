...
func (this *Player) foster_pull_cats_from_friend() (cats_in_friend []*msg_client_message.FosterCatInFriendInfo) {
	...
	for i := 0; i < len(cat_ids); i++ {
		...
		if friend != nil {
			...
			result := this.rpc_foster_get_cat_on_friend2(friend_id, cat_ids[i])
			if result == nil {
	...
	for i := 0; i < len(all_index); i++ {
		...
		if remain_seconds <= 0 {
			...
			if friend != nil {
				...
				if this.rpc_foster_settlement_to_friend(all_index[i], cat_id, cat_exp, items) == nil {
...
func fostered_slot_for_level(level int32) int32 {
	player_level := cfg_player_level_mgr.Map[level]
	if player_level == nil {
		return 0
	}
...
func (this *Player) fostered_slot_num_for_friend() int32 {
	...
	return fostered_slot_for_level(lvl)
}
	...
	if friend_ids != nil || len(friend_ids) > 0 {
		for i := 0; i < len(friend_ids); i++ {
			...
			if friend != nil {
				...
				result := this.rpc_foster_get_empty_slot_friend_info2(friend_ids[i])
				if result == nil {
					log.Error("Player[%v] get friend[%v] info error by rpc", this.Id, friend_ids[i])
					continue
				}
				friend_name, friend_level, friend_head := GetPlayerBaseInfo(friend_ids[i])
				d := &msg_client_message.FriendInfo{
					PlayerId:  proto.Int32(friend_ids[i]),
					Level:     proto.Int32(friend_level),
					Name:      proto.String(friend_name),
					Head:      proto.String(friend_head),
...
func (this *Player) get_player_foster_cats(player_id int32) int32 {
	...
	if player != nil {
		...
		result := this.rpc_foster_get_player_foster2(player_id)
		if result == nil {
		...
		for i := 0; i < len(result.PlayerFriendCats); i++ {
			...
			player_name, player_level, player_head := GetPlayerBaseInfo(c.PlayerId)
			d := &msg_client_message.FosteredFriendCatInfo{
				...
				FriendName:    proto.String(player_name),
				FriendLevel:   proto.Int32(player_level),
				FriendHead:    proto.String(player_head),
			}
