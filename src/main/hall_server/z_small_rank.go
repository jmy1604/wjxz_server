...
func (this *SmallRankService) SetUpdateRank(id, val int32) (retrank, retval int32) {
...
func (this *SmallRankService) AddUpdateRank(id, val int32) (retrank, retval int32) {
	retrank = -1
...
func (this *SmallRankService) GetAllIfChanged() (bchg bool, records []*SmallRankRecord) {
	...
	records = make([]*SmallRankRecord, this.rank_len)
	for idx, val := range this.rankrecords {
		if nil == val {
			continue
		}
...
func (this *SmallRankService) FillAllIfChanged() (bchg bool, dbrankreords []dbSmallRankRecordData) {
	this.rank_lock.UnSafeRLock("SmallRankService.GetRankById")
	defer this.rank_lock.UnSafeRUnlock()
	...
	dbrankreords = make([]dbSmallRankRecordData, this.rank_len)
	for idx, val := range this.rankrecords {
		if nil == val {
			continue
		}
		...
		dbrankreords[idx].Id = val.id
		dbrankreords[idx].Rank = val.rank
		dbrankreords[idx].Val = val.val
	}
