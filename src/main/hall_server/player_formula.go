...
func (this *Player) exchange_formula(formula_id int32) int32 {
	...
	// 区域
	if f.UnlockAreas != nil {
		for _, a := range f.UnlockAreas {
			if !this.db.Areas.HasIndex(a) {
				log.Error("Player[%v] area[%v] not unlock", this.Id, a)
				return int32(msg_client_message.E_ERR_FORMULA_EXCHANGE_NEED_AREA_UNLOCK)
			}
		}
	}
