...
func (this *Player) ChkDayExpeditionChgCount() (cur_count int32, cur_cost int32) {
	...
	if cur_count >= global_config_mgr.GetGlobalConfig().ExpeditionDayFreeChgCount {
		cur_cost = (cur_count + 1 - global_config_mgr.GetGlobalConfig().ExpeditionDayFreeChgCount) * global_config_mgr.GetGlobalConfig().ExpeditionDayChgAddCost
		if cur_cost > global_config_mgr.GetGlobalConfig().ExpeditionDayMaxChgCost {
			cur_cost = global_config_mgr.GetGlobalConfig().ExpeditionDayMaxChgCost
		}
	}
...
func (p *Player) CheckUpdateExpedition() {
	...
	if cur_count < global_config_mgr.GetGlobalConfig().ExpeditionTaskCount {
		new_tasks := cfg_expedition_mgr.RandNWithExistIds(cur_ids, p.db.Info.GetLvl(), global_config_mgr.GetGlobalConfig().ExpeditionTaskCount-cur_count)
...
func reg_player_expedition_msg() {
	log.Info("每日探险任务最多次数 %d", global_config_mgr.GetGlobalConfig().ExpeditionDayStartCount)
	msg_handler_mgr.SetPlayerMsgHandler(msg_client_message.ID_C2SGetAllExpedition, C2SGetAllExpeditionHandler)
	msg_handler_mgr.SetPlayerMsgHandler(msg_client_message.ID_C2SChgExpedition, C2SChgExpeditionHandler)
	msg_handler_mgr.SetPlayerMsgHandler(msg_client_message.ID_C2SStopExpedition, C2SStopExpeditionHandler)
	msg_handler_mgr.SetPlayerMsgHandler(msg_client_message.ID_C2SGetExpeditionReward, C2SGetExpeditionRewardHandler)
	msg_handler_mgr.SetPlayerMsgHandler(msg_client_message.ID_C2SStartExpedition, C2SStartExpeditionHandler)
	msg_handler_mgr.SetPlayerMsgHandler(msg_client_message.ID_C2SChgExpeditionResult, C2SChgExpeditionResultHandler)
}

func C2SGetAllExpeditionHandler(w http.ResponseWriter, r *http.Request, p *Player, msg proto.Message) int32 {
	...
	res_2cli.CurChgCount = proto.Int32(global_config_mgr.GetGlobalConfig().ExpeditionDayFreeChgCount - cur_count)
	if res_2cli.GetCurChgCount() < 0 {
		res_2cli.CurChgCount = proto.Int32(0)
	}
	res_2cli.CurChgCost = proto.Int32(cur_cost)
	res_2cli.CurChgCost = proto.Int32(cur_cost)
...
func C2SStartExpeditionHandler(w http.ResponseWriter, r *http.Request, p *Player, msg proto.Message) int32 {
	if nil == p {
		return 0
	}

	req := msg.(*msg_client_message.C2SStartExpedition)
	if nil == req {
		return 0
	}
	...
	task_cfg := cfg_expedition_mgr.Map[task_db.TaskId]
	if nil == task_cfg {
		return int32(msg_client_message.E_ERR_EXPEDITION_NO_TASK)
	}
	...
	if p.GetTodayExpeditionCount() >= global_config_mgr.GetGlobalConfig().ExpeditionDayStartCount {
	...
	var cat_cfg *table_config.XmlCharacterItem
	var c_db_len, c_cfg_len int32
	all_catdbs := make([]*dbPlayerCatData, 0, tmp_len)
	all_catcfgs := make([]*table_config.XmlCharacterItem, 0, tmp_len)
	...
	if rand.Int31n(10000) < expe_avi*task_cfg.SucceedBaseRate { // /10000
		...
		res_2cli.Result = proto.Int32(1)
	} else {
		res_2cli.Result = proto.Int32(0)
	}

	// 随机任务特殊事件
	var tmp_event *dbExpeditionEventData
	var tmp_e_cfg *table_config.XmlExpeditionEventItem
	tmp_sec := global_config_mgr.GetGlobalConfig().ExpeditionSPEventSec
	...
	for cur_sec := tmp_sec; cur_sec < task_cfg.CostTime; cur_sec += tmp_sec {
		...
		tmp_e_cfg = cfg_expedition_mgr.RandEvent(task_cfg.SearchEventId)
		if nil == tmp_e_cfg {
	...
	for _, tmp_e := range event_map {
		...
		msg_event.Sec = proto.Int32(tmp_e.Sec)
		msg_event.EventId = proto.Int32(tmp_e.ClientId)
	...
	res_2cli.Id = proto.Int32(task_id)
	res_2cli.CatIds = catids
	res_2cli.ExpeditionLeftSec = proto.Int32(task_cfg.CostTime)
	res_2cli.CurState = proto.Int32(PLAYER_EXPEDITION_STATE_DOING)
...
func C2SChgExpeditionHandler(w http.ResponseWriter, r *http.Request, p *Player, msg proto.Message) int32 {
	if nil == p {
		return 0
	}

	req := msg.(*msg_client_message.C2SChgExpedition)
	if nil == req {
		return 0
	}
	...
	log.Info("当前探险任务刷新次数%d  %d", cur_count, global_config_mgr.GetGlobalConfig().ExpeditionDayFreeChgCount)
...
func C2SStopExpeditionHandler(w http.ResponseWriter, r *http.Request, p *Player, msg proto.Message) int32 {
	if nil == p {
		return 0
	}

	req := msg.(*msg_client_message.C2SStopExpedition)
	if nil == req {
		return 0
	}
	unique_id := req.GetId()
	...
	p.db.Expeditions.FillAllClientMsg(res_2cli)
	cur_count, cur_cost := p.ChkDayExpeditionChgCount()
	res_2cli.CurChgCount = proto.Int32(cur_count)
	res_2cli.CurChgCost = proto.Int32(cur_cost)
...
func C2SGetExpeditionRewardHandler(w http.ResponseWriter, r *http.Request, p *Player, msg proto.Message) int32 {
	if nil == p {
		return 0
	}
	...
	req := msg.(*msg_client_message.C2SGetExpeditionReward)
	if nil == req {
		return 0
	}
	...
	max_cfg_len := int32(len(global_config_mgr.GetGlobalConfig().ExpeditionMultiVal))
	...
	RewardTotoalPercent := global_config_mgr.GetGlobalConfig().ExpeditionMultiVal[cat_num-1]

	tmp_len := int32(len(task_cfg.FixRewards))
	var obj_id, obj_num int32
	res_2cli_fi := &msg_client_message.S2CGetExpeditionReward{}
	res_2cli_fi.Id = proto.Int32(unique_id)
	...
	for idx := int32(0); idx+1 < tmp_len; idx = idx + 2 {
		...
		msg_item = &msg_client_message.IdNum{Id: proto.Int32(obj_id), Num: proto.Int32(obj_num)}
	...
	if PLAYER_EXPEDITION_RESULT_SUCCEED == task_db.Result {
		...
		var event_cfg *table_config.XmlExpeditionEventItem
		var tmp_event *dbExpeditionEventData
		res_2cli_fi.Specials = make([]*msg_client_message.IdNum, 0, task_db.TotalSpecials)
		for idx := int32(0); idx < tmp_len; idx++ {
			tmp_event = &task_db.EventIds[idx]
			event_cfg = cfg_expedition_mgr.EventMap[tmp_event.EventId]
			if nil == event_cfg {
	...
	p.Send(res_2cli_tasks)

	p.TaskUpdate(table_config.TASK_FINISH_EXPLORE_NUM, false, 0, 1)
	p.TaskUpdate(table_config.TASK_FINISH_EXPLORE_TASK_NUM, false, 0, 1)

	return 1
}

func C2SChgExpeditionResultHandler(w http.ResponseWriter, r *http.Request, p *Player, msg proto.Message) int32 {
	if nil == p {
		return 0
	}

	req := msg.(*msg_client_message.C2SChgExpeditionResult)
	if nil == req {
		return 0
	}
	...
	//p.RemoveResources(task_cfg.BuyBackCosts, "chgresult", "expedition")
	//p.db.Expeditions.SetCurState(task_id, PLAYER_EXPEDITION_STATE_SUCCEED)

	//res2cli := &msg_client_message.S2CRetAllExpedition{}
	//p.db.Expeditions.FillAllClientMsg(res2cli)
	//p.Send(res2cli)
